...
export default function ListPage({
  ...
  const handleOrderChange = (productId: number, newOrder: string) => {
    ...
    const existingProductId = Object.keys(currentOrders).find(
      (id) =>
        parseInt(id) !== productId && currentOrders[parseInt(id)] === orderValue
    );
  ...
  const handleUpdateOrder = async () => {
    ...
    const invalidOrders = Object.entries(featuredOrders).filter(([, order]) => 
      order < 1 || order > 1000
    );
                  ...
                  return (
                     <div className="w-20">
                       <InputField
                         ...
                         onChange={(e) =>
                           handleOrderChange(product.id, e.target.value)
                         }
  ...
  const handleDeleteProduct = async (id: number) => {
    ...
    if (
      confirm(
        "Are you sure you want to delete this product? This will affect all future bookings."
      )
    ) {
      try {
  ...
  return (
    <div className="space-y-6">
      ...
      <ComponentCard
        title={isFeaturedMode ? "Featured Products Order" : "Products"}
      >
        <DataTable
